...
def index(request):
	...
	for spreadsheetId in input_spreadsheets:
		_, body = gauth.getSheet(http, spreadsheetId, sheet_range, sheet_no=1)
		if "values" in body:
			input_mat.extend(body["values"])
		else:
			return HttpResponse("Available_keys: <br/>{}".format(body))

	output = "Read data from Sheet:{} <br/> {}".format(', '.join(input_spreadsheets), json.dumps(input_mat))
